...
We report basic planet information to give our users general information of when, by whom and how the planets were discovered. Since \cite{Wright2011}, as more planets were discovered by different methods other than radial velocity and transit, we have expanded this information to better ingest those planets into our database. 
...
KOI: KOI represents `Kepler Objects of Interest', which are planet candidates that have not been confirmed. 

KDE


EOD: EOD flag indicates planets with good Keplerian orbits. They have robust orbital parameters such as PER, ECC, K, OM, and I. 

\textit{(Sharon's comments: You don't need to
  describe why we include a planet in EOD -- this is covered at the
  beginning of Section~\ref{sec:update} as we discuss the `scope of
  EOD'. Just say what this flag is. Also, right now it includes RV,
  Transit, and also some Timing planets. The first sentence sounds
  like as long as a planet is discovered via RV and transit, it'll be
  in EOD, which is not true, I think.)}
...
methods other than radial velocity and transit. Since planets detected by these methods do not have robust orbital parameters, they are categorized as ``Other Planets'' on exoplanets.org. However, if planets have follow up radial velocity or transit observations and thus have good orbital parameters, they have EOD flags. See Section~\ref{sec:kepler} for more information about `Other Planets'.
...
We report distance of the host star in the unit of pc. 
...
We now report the month when the peer-reviewed paper is accepted to ApJ to better track of the trend in exoplanetary detections. Moreover, we report the discovery methods for the host star and the planet separately: STARDISCMETH and PLANETDISCMETH. See Section~ for more details. 
