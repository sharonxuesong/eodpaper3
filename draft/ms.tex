...
Restrict the updates in this section on EOD updates. We only report
new additions and revised fields here. For details on other fields,
see \cite{Wright2011}. We divide the fields into categories in
consistency with the interactive table on exoplanets.org. Some fields
are not explicitly included in the interactive table (e.g., the URLs), but are
explicitly listed in the downloadable {\tt .csv} file.\footnote{The
  entire dataset provided by exoplanets.org is available for download in a
  single CSV file at \url{http://exoplanets.org/csv-files/exoplanets.csv}}
...
(new) fields in stellar parameters: V\footnote{ZZZ VREF and VURL are
  also for J, H, KS. ZZZ}, VSINI, GAMMA; all orbit pars except for
...
We now include \kepler\ candidates...

We also include planets discovered microlensing, imaging, timing,
astrometric motion. Some microlensing and imaging planets are not
included in the EOD because of poorly characterized orbits...
...
SEDEPTHKS\dotfill & & Secondary eclipse depth in $K_{\rm S}$
band & -UPPER, etc. \\
...
\tablenotetext{b}{XU, XUPPER, XLOWER, XREF, XURL etc.}
