...
EOD, but now we also have planets discovered via timing. For the future, exoplanets discovered by any other methods with robust detections and good orbital parameters will also be included in the
EOD. Previously, we required the planets to be smaller than 24 \mjup\ but now we require the mass ratio to be smaller than 24 \mjup$/\msun$.
...
orbits (though these are not strictly quantitative; see Section~2 of
\citealt{Wright2011}). When new evidence comes to light and we are no
longer confident about the planet, we remove it from EOD, even if the
evidence is not peer reviewed.
