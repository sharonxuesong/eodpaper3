...
from less than 200 to over 1000 today (exoplanets.org). Before the
launch of NASA's \kepler\ mission \citep{Borucki2010}, most confirmed
exoplanets were discovered via the precise radial velocity (RV)
method. However, since 2009, \kepler\ has contributed over $\sim$ 400
confirmed exoplanets (ZZZ need citation update once the $>300$
\kepler\ release comes out) as well as over 3000 exoplanet candidates
...
\textit {Sharon: you've changed `discovered' to `detected', but it's
  ...
  person. 
  old: Sharon's comments: Instead of saying `discovered', we should say `detected'
