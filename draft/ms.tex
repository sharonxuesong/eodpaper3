...
{\bf I} is the orbital inclination. Previously, only transiting systems have I measured but now, sometimes, non-transiting systems have I's reported too. ZZZ(add reference) For example, one microlensing system has I measured (OGLE-2006-BLG-109L). 
...
transiting systems, which is measured by, e.g., the Rossiter-McLaughlin
effect or planet transiting star spots ZZZ add references for RM effect and the star spot method. $\lambda$ is introduced because the true spin-orbit angle
...
definition of $\lambda$ in \cite{Fabrycky2009} where the
...
$y$-axis to the projected stellar rotational axis. $\lambda$ is not a newly added field but we describe it here because $\lambda$'s were not reported consistently since some literatures
report $\beta = -\lambda$ instead. We sorted these cases out and made
sure that our $\lambda$'s are following the definition of \cite{Fabrycky2009}.
...
We also report the upper limits (XUL) for planet mass (MSINI), eccentricity (ECC), and the velocity semi-amplitude (K) when the literatures report them. For example, the upper limits for ECC is reported when the orbit is circular, and the upper limits for K is reported when there is no radial velocity detection. If only
...
to ECCUL/2. Since not all authors define the upper limits in the same way (e.g. 1$\sigma$ or some fixed percentile), we encourage the readers to refer to the literature for more information. 
...
Previously, we only reported general references (e.g. FIRSTREF, FIRSTURL, ORBREF, and ORBURL), but now we report the references for most of the fields. 

ZZZ RA, DEC, and PAR do not have references but they are from the \textit{Hipparcos} by \cite{van Leeuwen2009}. $B-V$ (BMV) field does not have a reference but the values are from the \textit{Hipparcos} catalog. The fields J H and KS do not have references as well but they are from the 2MASS \citep{Skrutskie2006}. The KP field do not have reference but the values are from the Exoplanet Archive \citep{Akeson2013}. 

SHK and RHK are poorly maintained fields and they unreferenced at the moment though we plan to include
proper reference in the future. Most of the values are from \cite{Butler2006}. 
...
Since \cite{Wright2011}, as more planets were discovered by different methods other than radial velocity and transit, we have expanded this information to better ingest those planets into our database. We are going to explain what they are and the criteria we used to put them into the EOD. 

{\bf KOI}'s: Although KOI's are not counted as part of the EOD due to their status of being `candidates', we include them in our website so that these KOIs are put into the context of all confirmed exoplanets. To display the KOIs in the EDE table, check the 'Kepler' option on the upper left of the table (default selection only includes 'Orbit Database' and 'Other'). The KOIs listed in our table include two past Kepler data releases and the most up-to-date release from February 2014. The original data are from the NASA Exoplanet Archive.

ZZZ Ask Jason about their criteria. \\
We also included robustly detected imaging, microlensing, and timing planets and the criteria are the following (as set by the QUACBED): 

\begin{itemize}
\item Imaging planets: 
\begin{enumerate}
\item Planet-star mass ratio $q < 0.024$ ($< 24\ $\mjup\ for a solar mass star); and in general we require ($q+\sigma_q$) $< 0.024$, where $\sigma_q$ is the uncertainty in $q$.
\item SEP $<$ 100 AU $\times$ ($M_*/M_\odot$), where SEP is the semi-major axis a if a is known, and the projected separation otherwise.
\item Confidently detected: the detection is clearly of a real astrophysical source, and will unlikely later be found to have been spurious.
\item Confidently bound: in opinion of the QUACBED the object will not be later found to be unbound or a chance alignment.
...
\end{itemize}
Planets primarily detected by these methods do not have robust orbital parameters and they are categorized as ``Other Planets'' on
exoplanets.org. If planets have follow up radial velocity or transit observations and thus have good orbital parameters, they have the EOD flags. See Section~\ref{sec:kepler} for more information about `Other Planets'. These other planets can be displayed by check the `Other' option for the table. They are not included in the EOD (especially some microlensing and imaging planets) because of poorly characterized orbits.
...
\tablehead{
  ...
  \colhead{Related Fields\tablenotemark{b}} & \colhead{Ref.}
}
\startdata
%
\sidehead{\textbf{Discovery and References}}
NAME\dotfill & String & Name of planet & \nodata & W11 \\
OTHERNAME\tablenotemark{c}\dotfill & String & Other commonly used name of star & \nodata & \ref{sec:disc} \\
...
T0\dotfill & Double & Epoch of periastron in HJD\tablenotemark{d} & -UPPER, etc. & W11 \\
...
HJD\tablenotemark{d} & -UPPER, etc. & W11 \\
...
HJD\tablenotemark{d} & -UPPER, etc. & \ref{sec:se} \\
%
...
column. They are... See Section~\ref{sec:unc} and \ref{sec:ref} for
more details. -UL stands for upper limits for the fields (see
