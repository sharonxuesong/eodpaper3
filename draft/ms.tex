...
The scope of the EOD has not been changed much but there was a major change in the category of host star. Previously, we only included exoplanets orbiting the `normal stars', but exoplanets orbiting neutron stars are also in our database. Furthermore, we only had exoplanets that were discovered by RV and transit but as more exoplanets are discovered by various methods such as microlensing and imaging, we have included those exoplanets as well into our database. 

We still require the ``planets" to be smaller than 24 \mjup\ and the robust detection, which although is not strictly quantitative. 

The purpose of the EOD has not changed; we aim to provide the readers with comprehensive database for exoplanets that are discovered up-to-date. However, we just do not put all the announced exoplanets as we are not trying to do ``an encyclopedic presentation of every claimed detection of an exoplanet" as mentioned in \cite{Wright2011}.
